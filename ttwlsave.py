...
import datalib
import OakSave_pb2, OakShared_pb2
import argparse
...
class TTWLSave(object):
    ...
    def import_protobuf(self, data):
        ...
        for e in self.save.equipped_inventory_list:
            ...
            slot = equip.get_obj_name # slotobj_to_slot[equip.get_obj_name()]
...
def parse_args():
    ...
    parser.add_argument('-o',type=str,help='Output Save File')
    parser.add_argument('--name',type=str,default=None,help='Change the name of the character to')
    parser.add_argument('--autoname',action='store_true',default=False,help='Rename part of the character name to the GUID')
    return parser.parse_args()

if __name__ == "__main__":
    args = parse_args()
    print(f"Loading {args.i}")
    save = TTWLSave( args.i )
    print(f'Character: {save.get_char_name()} GUID: {save.get_savegame_guid()}')
    guid = gen_ascii_guid()
    print(f"Changing guid to {guid}")
    save.set_savegame_guid( guid )
    if args.name is not None:
        print(f'Renaming Character: {save.get_char_name()} to {args.name}')
        save.set_char_name( args.name )
    elif args.autoname:
        name = save.get_char_name()
        if (len(name) > 4):
            name = name[0:-4] + guid[0:4]
        else:
            name = guid[0:4]
        print(f'Renaming Character: {save.get_char_name()} to {name}')
        save.set_char_name( name )
